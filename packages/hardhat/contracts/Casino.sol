...
// 3.commits Side A to the value randomA, because while no one can guess the value of randomA, once side A provides it everyone can check that its value is correct
// 4.Side B sends a message with another random number, randomB
// 5.Side A reveals the value of randomA in a third message
// 6.Both sides accept that the random number is randomA ^ randomB, the exclusive or (XOR) of the two values
